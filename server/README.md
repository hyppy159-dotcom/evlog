...
4. **Container Manager → 프로젝트 → 생성**
   ...
   - 빌드가 끝나면 컨테이너가 뜹니다.
5. 같은 NAS 안에서 확인: 브라우저로 `http://NAS내부IP:8127` →
   **아이디와 비밀번호를 정하는 화면**이 뜨면 성공입니다. 여기서 한 번만 만들면 됩니다.
...
4. 공유기에서 **443 포트만** NAS로 포워딩합니다. 8127은 열지 않습니다.
5. 브라우저로 `https://evlog.내도메인` 접속 → 아이디·비밀번호로 로그인.
...
자물쇠가 두 개입니다. 서로 다른 것이니 헷갈리지 마세요.

| | 무엇 | 어디에 넣나 |
|---|---|---|
| **앱** (차·휴대폰) | `EVLOG_TOKEN` — 긴 무작위 문자열 | `docker-compose.yml`, 그리고 앱 설정의 *접속 토큰* |
| **웹페이지** | 아이디 / 비밀번호 | 웹페이지를 처음 열 때 화면에서 정함 |

- **토큰을 길게.** 32자 이상 무작위.
- **비밀번호도 길게.** 8자가 최소일 뿐, 다른 데서 쓰던 것은 쓰지 마세요.
  비밀번호는 PBKDF2-SHA256으로 20만 번 늘려 해시해 저장하며, 원문은 서버에 남지 않습니다.
- **HTTPS로만.** `http://`로 열면 토큰과 비밀번호가 그대로 지나갑니다.
...
- 토큰을 바꾸려면 `docker-compose.yml`을 고치고 프로젝트를 다시 빌드하면 됩니다.
  앱 설정에서도 같이 바꿔 주세요.
- 웹 비밀번호는 대시보드 아래 **비밀번호 바꾸기**로 바꿉니다. 바꾸면 다른 기기의 로그인은 모두 끊깁니다.
- **비밀번호를 잊었다면** `docker-compose.yml`의 `EVLOG_RESET_LOGIN` 을 `"1"` 로 바꾸고
  프로젝트를 다시 빌드하세요. 계정만 지워지고 주행·충전 기록은 그대로 남습니다.
  웹페이지에서 새로 만든 뒤 다시 `"0"` 으로 되돌려 두세요.
...
| 웹에서 비밀번호가 틀리다고 함 | 여러 번 틀리면 10분 막힙니다. 잊었으면 위의 `EVLOG_RESET_LOGIN` |
| 앱이 토큰이 안 맞다고 함 | `docker-compose.yml`의 `EVLOG_TOKEN`과 앱에 넣은 값이 같은지 (앞뒤 공백 주의) |
...
```bash
EVLOG_TOKEN=test EVLOG_DB=/tmp/evlog.db EVLOG_PORT=8127 python3 server.py
python3 selftest.py http://127.0.0.1:8127 test    # API가 제대로 도는지 자체 점검
```
...
인증은 `Authorization: Bearer <EVLOG_TOKEN>` 헤더(앱) 또는 로그인 쿠키(웹).

| 메서드 | 경로 | 설명 |
|---|---|---|
| GET | `/auth/state` | 계정을 만들어야 하는지 / 로그인돼 있는지 |
| POST | `/setup` | 계정이 없을 때 딱 한 번. `{user, pass}` |
| POST | `/login` | `{user, pass}` → 세션 쿠키 (30일) |
